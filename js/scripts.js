$(() => {
    ...
    function timing_b2_2_wrap() {
        ...
        if (timing !== curTiming) {
            document
                .querySelector(".b2_2_wrap")
                .setAttribute("data-timing", curTiming);
            timing = curTiming;
        }
    }
    ...
    function setMaxSlide() {
        let maxSlide =
            document.querySelectorAll(".news_slider .owl-item").length -
            (document.querySelectorAll(".news_slider .owl-item.active").length -
                1);
        document
            .querySelector(".news_slider")
            .setAttribute("data-maxSlide", maxSlide);
        document
            .querySelector(".news_slider")
            .setAttribute("data-w", $(".news_slider .owl-stage-outer").width());
    }
    ...
    if (document.querySelector(".bg-to-change")) {
        ...
        hq.matches
            ? (curImg = "images/b3_bg.jpg")
            : (curImg = "images/mob_bg_shop.png");
        settedImg = curImg;
        document
            .querySelector(".paralax .bg-to-change")
            .setAttribute("src", settedImg);
        window.addEventListener("resize", () => {
            hq.matches
                ? (curImg = "images/b3_bg.jpg")
                : (curImg = "images/mob_bg_shop.png");
    ...
    if (owlquery) {
        ...
        function owlChanger() {
            ...
            owlTimer = setInterval(() => {
                ...
                if (dotw == 100) {
                    dotw = 0;
                    setTimeout(nextOwl, 40, "next.owl.carousel");
                }
            }, 40);
        }
        ...
        function changeDots() {
            ...
            for (i = 0; i < dots.length; i++) {
                if (
                    dots[i].getAttribute("data-index") <
                    dotactive.getAttribute("data-index")
                ) {
    ...
    if (document.querySelector(".time_slider")) {
        ...
        $(".time_slider")
            .on("initialize.owl.carousel", function (el) {
                setTimeout(() => {
    ...
    function translateTimeSlider() {
        ...
        var p1 = "16px";
        var p2 = "24px";
        var hqq = window.matchMedia(
            "all and (max-width: 1025px) and (min-width: 767px)"
        );
            ...
            if (slider.clientWidth > document.documentElement.clientWidth) {
                if (hqq.matches) {
                    document
                        ...
                        .setAttribute(
                            "style",
                            "transform: translateX(-" + p2 + "); width: 100vw;"
                        );
                    document
                        .querySelector(".time_slider .owl-stage-outer")
                        .setAttribute("style", "padding-left: " + p2);
                } else {
                    document
                        .querySelector(".time_slider")
                        .setAttribute(
                            "style",
                            "transform: translateX(-" + p1 + "); width: 100vw; "
                        );
                }
                document
                    .querySelector(".time_slider .owl-stage-outer")
                    .setAttribute("style", "padding-left: " + p1);
            } else {
...
//sync2 fight slider (synced) 
    ...
    function syncPosition(el, current) {
        ...
        // if (current > end) {
        //     sync2b.data("owl.carousel").to(current, 100, true);
        // }
        // if (current < start) {
        //     sync2b.data("owl.carousel").to(current - onscreen, 100, true);
        // }
    }
    ...
    setTimeout(() => {
        if (
            document.querySelector(
                ".swiper-slide-active>.burger-popup__slide-image>img"
            )
        ) {
            slideimgH = document.querySelector(
                ".swiper-slide-active>.burger-popup__slide-image>img"
            ).clientHeight;
...
window.addEventListener("load", () => {
    if (document.querySelector(".nris_input")) {
        document.querySelector(".nris_input").addEventListener("focus", () => {
            const input = document.querySelector(".nris_input");
            const search = input.closest(".nr_items_top ");
            search.classList.add("_active");
        });
        document.querySelector(".nris_input").addEventListener("blur", () => {
            const input = document.querySelector(".nris_input");
            const search = input.closest(".nr_items_top ");
