$(document).ready(() => {
    ...
    const setTouchScrollingEventsOfScrollableSections = (event, section) => {
        if (section.classList.contains("active")) {
            ...
            const deltaY =
                startTouchYScrollableSection - endTouchYScrollableSection;
    ...
    const setScrollingEventsOfScrollableSections = () => {
        document
            .querySelectorAll("#pagepiling > .section")
            .forEach((section) => {
                if (section.classList.contains("pp-scrollable")) {
                    section.addEventListener("keydown", (event) => {
                        setArrowScrollingEventsOfScrollableSections(
                            event,
                            section
                        );
                    });
                    ...
                    section.addEventListener("touchstart", (event) => {
                        startTouchYScrollableSection =
                            event.changedTouches[0].clientY;
                    });
                    section.addEventListener("touchmove", (event) => {
                        setTouchScrollingEventsOfScrollableSections(
                            event,
                            section
                        );
                    });
    ...
    const initEventsScreen = () => {
        ...
        $(
            ".b1b_head>span:nth-child(3), .b1b_head>span:nth-child(1)"
        ).removeClass("slide_n_show");
    ...
    const setMainScreenWheelEvents = (event) => {
        if (!isScrolling) {
            const isUp =
                event.deltaY < 0 || event.detail < 0 || event.wheelDelta > 0;
            const isDown =
                event.deltaY > 0 || event.detail > 0 || event.wheelDelta < 0;
